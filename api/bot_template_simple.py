...
Uses __PLACEHOLDER__ style to avoid conflicts with literal curly braces.
"""

SIMPLE_LP_TEMPLATE = r'''#!/usr/bin/env python3
"""QuantX Simple LongPort Bot — places one limit buy, waits, cancels."""
import sys, os, math, decimal, time, logging, requests, traceback
...
try:
    ...
    while True:
        time.sleep(60)
        log.info("Bot standing by...")

except Exception as e:
    log.error("Bot failed: %s", e)
    log.error(traceback.format_exc())
    log.info("Bot staying alive despite error. Toggle OFF to stop.")
    while True:
        time.sleep(60)
'''
...
import sys, os, time, logging, requests, traceback
...
try:
    ...
    if not ib.isConnected():
        log.error("Not connected after connect() call")
        sys.exit(1)
    ...
    if not price:
        log.info("Trying historical data fallback...")
